/*
   Magma test suite -- char_flare.c
   Copyright (C) 2006-2007 Tx0 <tx0@strumentiresistenti.org>
  
   Create, save and reload a character device flare.
...
#	define FLAREPATH "/test_char"
	magma_flare_t *flare = magma_new_chardev_flare(FLAREPATH);
	if (flare == NULL) {
		dbg(LOG_ERR, DEBUG_ERR, "Error creating char device flare!");
		exit(1);
	}
