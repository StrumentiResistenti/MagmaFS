	...
	magma_flare_response *response)
{
	...
	gchar *ptr = magma_format_request_header(buffer, MAGMA_OP_TYPE_OPEN, uid, gid, &tid, MAGMA_TERMINAL_TTL);

	ptr = magma_serialize_32(ptr, flags);
	ptr = magma_serialize_string(ptr, path);

	magma_log_transaction(MAGMA_OP_TYPE_OPEN, tid, peer);
	magma_send_and_receive(socket, peer, buffer, ptr - buffer, magma_pktar_open, response);

	return (tid);
}

void magma_pktqr_open(gchar *buffer, magma_flare_request *request)
{
	gchar *ptr = buffer;
	ptr = magma_deserialize_32(ptr, &request->body.open.flags);
...
GIOStatus magma_pktar_open(GSocket *socket, GSocketAddress *peer, magma_flare_response *response)
{
	gchar buffer[MAGMA_MAX_BUFFER_SIZE];
	memset(buffer, 0, MAGMA_MAX_BUFFER_SIZE);
	...
	if (response->header.res isNot -1) {
		ptr = magma_deserialize_string(ptr, response->body.open.commit_url);
	}
